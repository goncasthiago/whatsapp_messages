...
## Visão geral
- Backend: Python (conector para Supabase/Postgres).
- Banco: PostgreSQL no Supabase.
- Envio de mensagens: Z-API (integração a implementar).
- Frontend: previsto React (ainda não implementado).
...
- `get_contacts()`
- `insert_contact(display_name, phone_number, opt_in)`
- `get_contact_by_name(name)`

## Estrutura do conector
A classe principal do conector está em [api/api_connector.py](api/api_connector.py). Ela encapsula a criação do cliente Supabase e fornece métodos para leitura/inserção de contatos.

## Próximos passos / TODOs
- Implementar integração com Z-API para envio automático de mensagens.
